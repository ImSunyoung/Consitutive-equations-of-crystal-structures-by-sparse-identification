...
type = 1;
materials = {'Si','Ge'};
refer_data = sprintf('Reference_data_cubic_%s.mat',materials{type});
load(refer_data);
...
save_name = sprintf('data/constitutive_coeff_cubic_%s.mat', materials{type});
...
for i = 1:nStep
    ...
    [var_out] = func_cubic_predict(fid, short_coeff_sparse_id, matF);
    if fid == 1
