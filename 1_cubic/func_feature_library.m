...
% order computation 
od_invariant = [1 2 3 2 4 3 3 5 4];
...
for i = 1:nPoint
    ...
    matG = matC - eye(3);
    %----------------------------------------------------------
    JJ_temp = get_invariant_cubic(matG,0);        % scalar      ~ 9
    JJ_C_temp = get_invariant_cubic(matG,1);      % vector 6*1  ~ 9
    JJ_CC_temp = get_invariant_cubic(matG,2);     % matrix 6*6  ~ 9
    JJ = zeros(1,9);
    JJ_C = zeros(6,9);
    JJ_CC = zeros(6,6,9);
    for tp = 1: 9
