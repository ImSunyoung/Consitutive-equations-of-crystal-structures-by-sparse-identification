...
type = 1;
materials = {'TiO2','ZnO'};
refer_data = sprintf('Reference_data_tetra_%s.mat',materials{type});
load(refer_data);
...
% save_name = sprintf('data/constitutive_coeff_tetra_%s.mat', materials{type});
...
for i = 1:nStep
    ...
    [var_out] = func_tetragonal_predict(fid, short_coeff_sparse_id, matF);
    if fid == 1
