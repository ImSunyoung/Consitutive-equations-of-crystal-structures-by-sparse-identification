...
%% the number of used components of tangent modulus
nCP =  9;
...
% material_coeff = zeros(17,4);
% save Cu_material_coeff.mat material_coeff
...
% ncoeff_2 = 8;
% ncoeff_3 = 3;
% ncoeff = ncoeff_1 + ncoeff_2 + ncoeff_3;
...
%% The compoenet of coefficient matrix A 
...
for i = 1:nPoint
    ...
    matG = matC - eye(3);
    %----------------------------------------------------------
    JJ_temp = get_invariant_tetra2(matG,0);        % scalar      ~ 9
    JJ_C_temp = get_invariant_tetra2(matG,1);      % vector 6*1  ~ 9
    JJ_CC_temp = get_invariant_tetra2(matG,2);     % matrix 6*6  ~ 9
    JJ = zeros(1,8);
    JJ_C = zeros(6,8);
    JJ_CC = zeros(6,6,8);
    for tp = 1: 8
